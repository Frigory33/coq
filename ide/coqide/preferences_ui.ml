...
let configure ?(apply = fun () -> ()) parent =
  ...
  let config_modifiers =
    create_pref_box ~in_grid:true "Modifiers for menu item accelerators" [
...
*)
  ...
  if edit_preferences ~apply "Preferences" ~parent ~current_section ~width:600 ~height:400 cmds then
    save_pref ();
