...
let build_ui () =
  ...
  menu edit_menu [
    ...
    item "Select All" ~accel:"<Primary>a" ~stock:`SELECT_ALL
      ~callback:(cb_on_current_term (fun t ->
         t.script#select_all (); t.proof#select_all (); t.messages#select_all));
    item "Cut" ~stock:`CUT
  ...
  menu debug_menu [
    ...
    item "Toggle breakpoint" ~label:"_Toggle breakpoint" ~accel:"F8"
      ~stock:`MEDIA_RECORD ~callback:MiscMenu.toggle_breakpoint;
    item "Continue" ~label:"_Continue" ~accel:"F9"
      ~stock:`MEDIA_NEXT ~callback:Nav.continue;
    item "Step in" ~label:"Step in" ~accel:"F10"
      ~stock:`MEDIA_PLAY ~callback:Nav.step_in;
    item "Step out" ~label:"Step out" ~accel:"<Shift>F10"
      ~stock:`MEDIA_FORWARD ~callback:Nav.step_out;
    (* todo: consider other names for Break and Interrupt to be clearer to users *)
    item "Break" ~label:"Break" ~accel:"F11"
      ~stock:`MEDIA_PAUSE ~callback:Nav.break;
