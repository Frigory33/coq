...
A dark GTK theme is used, the Oblivion style scheme is used for syntax highlighting,
and custom colors have been set.
...
The lower right panel has three tabs:
...
for menu items are shown on the right of each item. You can
:ref:`customize key bindings <coqide_key_bindings>`.

If you need more complex editor commands, you can launch an external text editor on the
current buffer, using the *Edit/External Editor* menu.
Use *Edit/Preferences/Externals/External Editor* to specify the external text editor.
When you're done editing, if auto revert is enabled, CoqIDE will ask you if you want
to reload the file, otherwise you must reopen the file to see your changes.

Like a lot of editing software, CoqIDE supports the following commands:

- Undo is available as `Ctrl-Z` (`Cmd-Z` on macOS).  Redo is `Ctrl-Shift-Z`
...
- `Ctrl-Backspace` deletes a word of text before the cursor, while
...
- `Tab` and `Shift-Tab` indent or unindent the currently selected lines of code.
...
*auto revert* feature: in case an opened file is modified on disk by a
...
The *Shortcuts* section (screenshot below) contains buttons that allow you
to change the modifiers (`Ctrl`, `Alt`, `Cmd`, `Shift`) used for the
key bindings of all items of a menu (except the View menu in which only
the togglable items are concerned). To make the interface cleaner,
all available modifiers are presented at the top. You may toggle
some of them to be able to use them as modifiers.
Once you apply your changes, you will see in the menu how accelerators
have changed. Note that, in case of conflict between menus,
accelerators will be removed and you will need to use one of the methods
described in the next section to get them back.

The *Misc* section has to be documented...
...
which was bound by default to `Shift-Ctrl-A`.  `<Primary>` binds to `Cmd` on macOS
and `Ctrl` on other systems. The second line is for a menu item that has no key binding.
...
last line is used.  This is convenient for copying a group of changes from elsewhere –
just insert the changes at the end of the file.  The next time CoqIDE terminates, it will
resort the items.
...
Key bindings that don't appear in the file such as `Shift-Tab` (unindent)
can't be changed with this mechanism.
